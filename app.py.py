...
if es_cliente:
    ...
    tabs = st.tabs(["📊 Stock", "⚙️ Maestro", "👥 Cta Cte", "📄 Presupuestador", "📋 Movimientos", "🏁 Cierre"])
    ...
    with tabs[1]: # MAESTRO
        ...
        if st.button("Guardar Cambios en Google Sheets"):
            guardar_hoja(sh_stock, df_ed)
            st.success("¡Datos actualizados!"); st.rerun()

    with tabs[2]: # CTA CTE - COMPLETO
        ...
        if not df_clientes.empty:
            ...
            for i, row in hist.iterrows():
                ...
                with st.expander(f"{tipo_col} {row['Fecha']} | {row['Tipo']} | {formatear_moneda(row['Monto'])}"):
                    ...
                    if row["Tipo"] in ["VENTA", "N. CRÉDITO"]:
                        # Lógica para reconstruir carrito
                        temp_car = []
                        ...
                        if temp_car:
                            ...
                            st.download_button(f"🖨️ Descargar Comprobante {i}", pdf_r, f"Comprobante_{i}.pdf", "application/pdf")
            ...
            if st.button("Confirmar Pago"):
                ...
                nuevo_m = pd.DataFrame([{"Fecha": datetime.now().strftime("%d/%m/%Y"), "Cliente": cli_sel, "Tipo": "PAGO", "Monto": monto_p, "Metodo": "Efectivo", "Detalle": "Pago registrado"}])
                guardar_hoja(sh_movs, pd.concat([df_movs, nuevo_m]))
                st.success("Pago guardado!"); st.rerun()

        st.subheader("Nuevo Cliente")
        with st.expander("Dar de alta"):
            n_n = st.text_input("Nombre"); n_t = st.text_input("Tel")
            if st.button("Crear Cliente"):
                pd.concat([df_clientes, pd.DataFrame([[n_n, n_t, "", "", 0.0]], columns=COLS_CLI)], ignore_index=True).pipe(lambda d: guardar_hoja(sh_clientes, d))
                st.rerun()

    with tabs[3]: # PRESUPUESTADOR - COMPLETO
        st.header("Generador de Presupuestos / Ventas / Notas de Crédito")
        cli_p = st.selectbox("Cliente:", df_clientes["Nombre"].tolist() if not df_clientes.empty else ["Consumidor Final"], key="p_cli")
        ...
        if st.session_state.carrito:
            ...
            with b1:
                if st.button("✅ VENTA (Baja Stock)"):
                    ...
                    st.success("Venta realizada"); st.session_state.carrito = []; st.rerun()
            with b2:
                if st.button("🔵 NOTA CRÉDITO (Suma Stock)"):
                    for item in st.session_state.carrito:
                        df_stock.loc[df_stock["Accesorio"] == item["Producto"], "Stock"] += item["Cant"]
                    guardar_hoja(sh_stock, df_stock)
                    idx_cp = df_clientes[df_clientes["Nombre"] == cli_p].index[0]
                    ...
                    st.success("Nota de Crédito realizada"); st.session_state.carrito = []; st.rerun()
            with b3:
                if st.button("🗑️ Limpiar"):
                    st.session_state.carrito = []; st.rerun()

    with tabs[4]: # MOVIMIENTOS
        st.header("Historial Global de Movimientos")
        st.dataframe(df_movs, use_container_width=True, hide_index=True)

    with tabs[5]: # CIERRE
        st.header("Cierre de Caja")
        st.metric("Valorización Stock (Costo)", formatear_moneda((df_stock['Stock'] * df_stock['Costo Base']).sum()))
        st.metric("Deuda Total de Clientes", formatear_moneda(df_clientes['Saldo'].sum()))
