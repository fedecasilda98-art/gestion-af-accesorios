...
if es_cliente:
    ...
    with tabs[3]: # CTA CTE CON DESPLEGABLE DE ÓRDENES
        ...
        if not df_clientes.empty:
            ...
            with col_movs:
                ...
                if hist.empty:
                    ...
                    # Renderizado manual para permitir expansores por cada orden
                    for i, row in hist.iterrows():
                        color = "🔴" if row["Tipo"] == "VENTA" else "🟢"
                        # Título de la barra desplegable
                        titulo_barra = f"{color} {row['Fecha']} | {row['Tipo']} | {formatear_moneda(row['Monto'])}"
                        
                        with st.expander(titulo_barra):
                            st.write(f"**Método:** {row['Metodo']}")
                            st.write(f"**Detalle/Productos:**")
                            # Si el detalle tiene comas (nuestro formato de productos), lo listamos lindo
        ...
        with col_mod:
            with st.expander("✏️ Modificar Datos"):
                if not df_clientes.empty:
        ...
        with col_del:
            with st.expander("🗑️ Eliminar Cliente"):
                if not df_clientes.empty:
                    cli_d = st.selectbox("Borrar:", df_clientes["Nombre"].tolist(), key="s_del")
                    st.warning(f"¿Borrar a {cli_d}?")
    ...
    with tabs[4]: # PRESUPUESTADOR
        ...
        if st.button("Agregar al Carrito", key="btn_add_p"):
        ...
        if st.session_state.carrito:
            ...
            with b2:
                if st.button("✅ ORDEN DE TRABAJO", use_container_width=True):
                    # GUARDAMOS NOMBRE, CANTIDAD Y PRECIO UNITARIO DEL MOMENTO
                    det_productos = ", ".join([f"{item['Cant']}x {item['Producto']} (á {formatear_moneda(item['Precio U.'])})" for item in st.session_state.carrito])
