...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(35) # Espacio para que el texto no pise el logo
        self.cell(0, 10, "ACCESORIOS DE ALUMINIO", ln=True)
        self.ln(10)

# --- FUNCIÓN GENERADORA DE PDF ---
def generar_pdf_binario(cliente, carrito, total):
    pdf = PDF() # Usa la clase personalizada con el logo
    pdf.add_page()
...
if es_cliente:
    ...
    with tabs[4]: # PRESUPUESTADOR
        ...
        with p1:
            item_p = st.selectbox("Seleccionar Artículo:", df_stock["Accesorio"].tolist(), key="itm_p")
        with p2:
        ...
        if st.button("Agregar al Presupuesto", key="add_btn"):
        ...
        if st.session_state.carrito:
            ...
            with b2:
                if st.button("✅ ORDEN DE TRABAJO", key="order_btn", use_container_width=True):
    ...
    # El resto de las pestañas (Stock, Lote, Maestro, Cta Cte, Órdenes, Caja) se mantienen con el código de tu versión anterior.
