...
# --- CARGA DE DATOS CON ESCUDO DE COLUMNAS ---
...
# Definición de Columnas Oficiales
...
if es_cliente:
    ...
    with tabs[1]: # LOTE (COMPLETO)
        ...
        if st.button("Actualizar Stock por Lote"):
            st.info("Función de procesamiento de lote lista para conectar con tu lógica de cálculo.")
    ...
    with tabs[3]: # CTA CTE (COMPLETO CON MEJORAS)
        st.header("👥 Gestión de Cuentas Corrientes")
        if not df_clientes.empty:
            cli_sel = st.selectbox("🔍 Buscar Cliente para ver Estado e Historial:", df_clientes["Nombre"].tolist(), key="busqueda_global_cli")
            ...
            with st.expander("Editar Datos"):
                if not df_clientes.empty:
                    ...
                    if st.button("Guardar Cambios", key="b_mod"):
                        df_clientes.at[idx_e, "Nombre"], df_clientes.at[idx_e, "Saldo"] = e_n, round(e_s, 2)
        ...
        with col_del:
            ...
            with st.expander("Zona de Peligro"):
                if not df_clientes.empty:
                    ...
                    if st.checkbox("Confirmo eliminación permanente", key="c_del_c"):
                        if st.button(f"ELIMINAR A {cli_d}", type="primary"):
                            df_clientes[df_clientes["Nombre"] != cli_d].to_csv(ARCHIVO_CLIENTES, index=False)
                            st.rerun()

    with tabs[4]: # PRESUPUESTADOR (COMPLETO CON DETALLE)
        ...
        if st.session_state.carrito:
            ...
            with b1:
                ...
                st.download_button("📥 DESCARGAR PDF", pdf_d, f"Presupuesto_{cliente_p}.pdf", "application/pdf")
            with b2:
                if st.button("✅ ORDEN DE TRABAJO", use_container_width=True):
                    ...
                    if cliente_p != "Consumidor Final":
                        ...
                        df_clientes.to_csv(ARCHIVO_CLIENTES, index=False)
                        n_v = pd.DataFrame([{"Fecha": datetime.now().strftime("%d/%m/%Y %H:%M"), "Cliente": cliente_p, "Tipo": "VENTA", "Monto": t_f, "Metodo": "-", "Detalle": det_prod}])
